...
### 알고리즘의 한계점 논의 (15점 핵심 항목)
* **해상도 의존성**: 이미지의 크기에 따라 설정된 파라미터(9, 5 등)가 다르게 작용하여, 해상도별로 결과물의 선 굵기가 달라지는 한계가 있습니다.
* **복잡한 배경 노이즈**: 숲이나 복잡한 패턴이 있는 배경에서는 모든 미세한 선을 엣지로 인식하여 화면이 시커멓게 타버리는 현상이 발생합니다.
